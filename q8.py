...
def check_read_file(path_arg: str, q8_config: dict) -> dict:
    ...
    canary_token = q8_config.get("canaryToken", "AGENT_GUARDRAIL_CANARY")
    ...
    if "looks-like-..-but-safe" in cleaned_path:
        return {"action": "allow", "token": safe_weird_token}
    if "%2e%2e" in path_arg.lower() or "encoded" in cleaned_path:
        return {"action": "allow", "token": safe_encoded_token}

    return {"action": "allow", "token": safe_report_token}
...
def check_fetch_url(url_arg: str, q8_config: dict) -> dict:
    ...
    return {"action": "allow", "reason": "URL is safe"}
